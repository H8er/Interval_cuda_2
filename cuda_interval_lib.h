...
template<class T> inline __device__
interval_gpu<T> sin(interval_gpu<T> const &x)
{
  const T &xl = x.lower();
  const T &xu = x.upper();

  double pi2  = asin(1.0)*4;
  double pi05 = asin(1.0)/2;
  double a,b;

  if( ceil((xl - pi05)/pi2) <= floor((xu - pi05)/pi2)){
  b = 1;}
  else{
  b = max(sin(xl), sin(xu));}
  ...
  return interval_gpu<T>(a,b);
}

template<class T> inline __device__
interval_gpu<T> cos(interval_gpu<T> const &x)
{
  const T &xl = x.lower();
  const T &xu = x.upper();
  ...
  if( ceil((xl - pi)/pi2) <= floor((xu - pi)/pi2)){
  b = 1;}
  else{
  b = max(cos(xl), cos(xu));}
